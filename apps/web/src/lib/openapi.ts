  ...
  return {
    ...
    paths: {
      ...
      "/api/v1/features/{specId}/content": {
        parameters: [{ name: "specId", in: "path", required: true, schema: { type: "string" } }],
        put: { tags: ["features"], summary: "Replace a spec's Markdown body and commit it to the connected repo. Frontmatter (and so the stable id) is preserved.", requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["content"], properties: { content: { type: "string", description: "Markdown after the frontmatter." }, message: { type: "string", description: "Commit message; defaults to docs(specboard): update <path>." } } } } } }, responses: { "200": ok("The written spec: specId, path, commitSha."), "422": { $ref: "#/components/responses/Invalid" } } },
      },
      "/api/v1/specs": {
        post: { tags: ["features"], summary: "Create a spec file, commit it to a connected repo, and sync it onto the board. With workItemId the spec attaches to that existing leaf item, keeping its id, status, assignee, parent and history; with parentSpecId a new item is created and nested under that card. The two are mutually exclusive.", requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["title"], properties: { title: { type: "string" }, body: { type: "string", description: "Markdown body, no frontmatter. Defaults to a stub." }, workItemId: { type: "string", format: "uuid", description: "Existing leaf work item to attach the spec to." }, parentSpecId: { type: "string", format: "uuid", description: "Card to nest the newly created item under. Not valid with workItemId." }, repoId: { type: "string", description: "Target repository; defaults to the spec repo." }, message: { type: "string", description: "Commit message; defaults to docs(specboard): add <path>." } } } } } }, responses: { "201": ok("The created spec (specId, path, commitSha), plus parentWarning when the file was committed but nesting it failed."), "422": { $ref: "#/components/responses/Invalid" } } },
      },
      "/api/v1/statuses": {
