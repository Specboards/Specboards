  ...
  return {
    ...
    paths: {
      ...
      "/api/v1/features/{specId}/content": {
        parameters: [{ name: "specId", in: "path", required: true, schema: { type: "string" } }],
        put: { tags: ["features"], summary: "Replace a spec's Markdown body and write it back to the connected repo. Frontmatter (and so the stable id) is preserved. Where it lands follows the repo's writeMode: direct commits onto the default branch, pr (the default) commits to a working branch and proposes it, joining the pull request already open for that file.", requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["content"], properties: { content: { type: "string", description: "Markdown after the frontmatter." }, message: { type: "string", description: "Commit message; defaults to docs(specboard): update <path>." } } } } } }, responses: { "200": ok("The written spec: specId, path, commitSha, plus pullRequest (number, url, branch, created) when the change was proposed for review rather than committed. Its presence means the board still shows the previous text."), "422": { $ref: "#/components/responses/Invalid" } } },
      },
      "/api/v1/specs": {
