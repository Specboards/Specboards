...
export function buildOpenApiDocument(baseUrl: string): Record<string, unknown> {
  return {
    ...
    paths: {
      ...
      "/api/v1/statuses": {
        get: { tags: ["workflow"], summary: "The workspace's stages, the effective transition mode, plus the resolved workflow (statuses + legal transitions). Pass productId to resolve for one product; without it you get the workspace default.", parameters: [{ name: "productId", in: "query", required: false, schema: { type: "string", format: "uuid" }, description: "Resolve transitions for this product. Omit for the workspace default." }], responses: { "200": ok("Stages, transition mode, and workflow.") } },
        put: { tags: ["workflow"], summary: "Replace the workflow stages (the board's columns). With productId this sets that product's own stages (product admin or workspace owner); without one it sets the workspace default that unconfigured products inherit (owner only). Any item left in a stage that no longer exists is re-homed to the first stage. An empty array reverts a product to inheriting, and is refused without a productId.", requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["statuses"], properties: { statuses: { type: "array", minItems: 0, maxItems: 30, description: "Ordered stages, 2 to 30 of them. An empty array reverts a product to inheriting the workspace default.", items: { type: "object", required: ["label"], properties: { key: { type: "string", description: "Existing stage key, so a rename keeps the stage's items. Derived from the label when omitted. `archived` is reserved." }, label: { type: "string" } } } }, productId: { type: "string", format: "uuid", nullable: true, description: "Product to configure. Omit to set the workspace default." } } } } } }, responses: { "200": ok("The stages now in force for that scope, which for a revert is the inherited set."), "403": { $ref: "#/components/responses/Forbidden" }, "422": { $ref: "#/components/responses/Invalid" } } },
        patch: { tags: ["workflow"], summary: "Set a transition mode, strict or flexible. With productId this configures that product (product admin or workspace owner) and null reverts it to inheriting; without productId it sets the workspace default (owner only).", requestBody: { required: true, content: { "application/json": { schema: { type: "object", properties: { transitionMode: { type: "string", nullable: true, enum: ["strict", "flexible", null], description: "null reverts a product to inheriting the workspace default; not valid without productId." }, productId: { type: "string", format: "uuid", nullable: true, description: "Product to configure. Omit to set the workspace default." } }, required: ["transitionMode"] } } } }, responses: { "200": ok("The transition mode now in force, which for a revert is the inherited one."), "403": { $ref: "#/components/responses/Forbidden" }, "422": { $ref: "#/components/responses/Invalid" } } },
      },
      "/api/v1/products": {
