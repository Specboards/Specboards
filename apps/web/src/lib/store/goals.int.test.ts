...
describe.skipIf(!OWNER_URL)("goals, key results and linkage (store + RLS)", () => {
  ...
  it("computes key-result progress from start/current/target on read", async () => {
    ...
    updated = await store.updateKeyResult(krId, { currentValue: 50 }, asOwner);
  ...
  it("averages several key results for the goal's progress", async () => {
    ...
    const updated = await store.createKeyResult(
      goal.id,
      { title: "Untouched", startValue: 0, targetValue: 100, currentValue: 0 },
      asOwner,
    );
    expect(updated.progress).toBe(50);
  });
  ...
  it("is a no-op when the same link is made twice", async () => {
    ...
    await store.linkGoal(goal.id, item.specId, asOwner);
    expect(await store.listGoalContributions(goal.id, asOwner)).toHaveLength(1);
  });
  ...
  it("shows a viewer the goal but only the linked work they can read", async () => {
    ...
    // The owner sees both.
    expect(await store.listGoalContributions(goal.id, asOwner)).toHaveLength(2);
  ...
  it("removes a deleted feature's links and leaves the goal intact", async () => {
    ...
    expect(await store.listGoalContributions(goal.id, asOwner)).toHaveLength(0);
  });
  ...
  it("makes org-wide goals owner-only and product goals product-scoped", async () => {
    ...
    await expect(
      store.createGoal({ title: "Not mine", productId: product.beta }, asContributor),
  ...
  it("nests goals but refuses a cycle", async () => {
    ...
    const child = await store.createGoal(
      { title: "Product objective", productId: product.alpha, parentGoalId: parent.id },
      asOwner,
    );
  ...
  it("orphans child goals to the root rather than deleting them", async () => {
    ...
    const child = await store.createGoal(
      { title: "Orphan me", productId: product.alpha, parentGoalId: parent.id },
      asOwner,
    );
    await store.deleteGoal(parent.id, asOwner);

    const listed = (await store.listGoals(asOwner)).find((g) => g.id === child.id);
    expect(listed).toBeDefined();
